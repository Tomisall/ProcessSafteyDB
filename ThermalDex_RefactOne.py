...
from PyQt5.QtGui import QPixmap, QColor, QIcon, QRegExpValidator
	...
	#Add labels for calculated values
        ...
        def search_database():
             ...
             if entry_search.text() == '' or entry_search.text() == None:    
                 ...
                 self.text_search_type('name',entry_search.text())
                 show_result(self, self.selectedDatabase, True)

             elif self.searchTypeSelection.currentText() == 'Project':
                 self.text_search_type('proj',entry_search.text())
                 show_result(self, self.selectedDatabase, True)                
 
             elif self.searchTypeSelection.currentText() == 'MW':
                 self.value_search_type('MW',entry_search.text())
                 show_result(self, self.selectedDatabase, True)

             elif self.searchTypeSelection.currentText() == 'Qdsc':
                 self.value_search_type('Q_dsc',entry_search.text())
                 show_result(self, self.selectedDatabase, True)
                 
             elif self.searchTypeSelection.currentText() == 'Tinit':
                 self.value_search_type('initT',entry_search.text())
                 show_result(self, self.selectedDatabase, True)     

             elif self.searchTypeSelection.currentText() == 'Tonset':
                 self.value_search_type('onsetT',entry_search.text())
                 show_result(self, self.selectedDatabase, True)                 

             elif self.searchTypeSelection.currentText() == 'Td24':
                 self.value_search_type('Td24',entry_search.text())
                 show_result(self, self.selectedDatabase, True)                     

             elif self.searchTypeSelection.currentText() == 'O.R.E.O.S. at >500 g':
                 self.value_search_type('oreoLargeScale_val',entry_search.text())
                 show_result(self, self.selectedDatabase, True)

             elif self.searchTypeSelection.currentText() == 'Rule of Six':
                 self.value_search_type('RoS_val',entry_search.text())
                 show_result(self, self.selectedDatabase, True)

             elif self.searchTypeSelection.currentText() == 'Oxygen Balance':
                 self.value_search_type('OB_val',entry_search.text())
                 show_result(self, self.selectedDatabase, True)
